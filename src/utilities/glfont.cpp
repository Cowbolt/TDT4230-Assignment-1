...
Mesh generateTextGeometryBuffer(std::string text, float characterHeightOverWidth, float totalTextWidth) {
    ...
    for(unsigned int i = 0; i < text.length(); i++)
    {
        ...
        // 128 chars in ASCII
        mesh.textureCoordinates.at(4 * i + 0) = {text.at(i)/128.f, 0};
        mesh.textureCoordinates.at(4 * i + 1) = {text.at(i)/128.f + 1/128.f, 0};
        mesh.textureCoordinates.at(4 * i + 2) = {text.at(i)/128.f + 1/128.f, 1};
        mesh.textureCoordinates.at(4 * i + 3) = {text.at(i)/128.f, 1};
